...
const GuineeCredits = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        setLoading(true);
        const response = await fetch("/api/crud/guineeCredits/history");
        if (!response.ok) {
          throw new Error("Erreur lors de la récupération de l'historique");
        }
        const historyData: GuineeCreditHistory[] = await response.json();
  ...
  return <BizDataTable title='Historique des Crédits Guinée' data={data} />;
};

export default GuineeCredits;
