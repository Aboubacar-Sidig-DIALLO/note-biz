...
const Investments = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        setLoading(true);
        const response = await fetch("/api/crud/investments/history");
        if (!response.ok) {
          throw new Error("Erreur lors de la récupération de l'historique");
        }
        const historyData: InvestmentHistory[] = await response.json();
  ...
  return <BizDataTable title='Historique des Investissements' data={data} />;
};
